...
/**
 * Data class for dependency with vulnerabilities
 */
...
class OsVInspection : LocalInspectionTool() {
    ...
    private fun scanFileForDependencies(file: PsiFile, holder: ProblemsHolder) {
            ...
            if (parser.canHandle(fileName)) {
                try {
                    ...
                    dependencies.forEach { dep ->
                        ...
                        val vulnerabilities = apiService.queryVulnerabilities(
                            dep.name,
                            dep.ecosystem,
                            dep.version
                        )
    ...
    private fun shouldReportVulnerability(vuln: Vulnerability): Boolean {
        ...
        val config = com.intellij.openapi.components.ServiceManager.getService(OsVConfig::class.java)
    ...
    fun checkDependencies(dependencies: List<Dependency>): List<DependencyWithVulnerabilities> {
        ...
        dependencies.forEach { dep ->
            ...
            if (vulnerabilities.isNotEmpty()) {
                results.add(DependencyWithVulnerabilities(dep, vulnerabilities))
            }
        }
